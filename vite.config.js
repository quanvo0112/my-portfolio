  ...
  return {
    ...
    transformIndexHtml(html) {
      const script = [
        ...
        `var BUILT=${JSON.stringify(BUILD_VERSION)};`,
        ...
        `  fetch(${JSON.stringify(base)}+'version.json',{cache:'no-store'})`,
        `    .then(function(r){if(!r.ok)throw 0;return r.json()})`,
        `    .then(function(d){`,
        `      var v=d&&d.version,last=get();`,
        `      if(!v||v===last)return;`,
        `      set(v);`,
        `      var path=location.pathname+(location.pathname.endsWith('/')?'':'/')+'?v='+v;`,
